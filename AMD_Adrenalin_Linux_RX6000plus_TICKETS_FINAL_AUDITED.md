...
- [x] Add the typed Hardware1 device-list/static-info/capability contract,
  ...
  - [x] Settings1 toast-notifications production slice: a typed read/update API now
    ...
    reads return `configured=false` (with `enabled` ignored) until a value is
    explicitly saved. The generated D-Bus/private-bus service test verifies
    initialization, update, event envelope,
...
Progress (fixture version/trust-boundary policy; 2026-09-25): the fixture reader and producer now reject ABI minor versions other than 0 and nonzero reserved header fields. The contract records immutable-header and descriptor/mapping lifetime preconditions for future production opens, while explicitly noting that the pointer-level fixture cannot enforce descriptor properties. It clarifies that open-time size validation alone cannot prevent later truncation and leaves the production truncation control and compatible-minor negotiation as adoption requirements. Regression coverage expands malformed-header reader and producer cases; strict-warning ABI build and focused CTest pass 1/1. Reasoning-only independent review agreed this is fail-closed for the fixture and identified those same production adoption questions; it did not inspect the checkout. Telemetry remains fixture-only; production ABI design/review, producer, source arbitration, restart recovery, and UI remain open.
